...
class KameraPaneli(FloatLayout):
    ...
    def _kurulum(self):
        # ── Kamera: gizli widget, texture'i biz ciziyoruz ────────────────────
        # Camera widget'i gorsel olarak gizliyoruz (size 0),
        # texture'i Canvas'ta manuel Rectangle olarak ciziyoruz.
        # Bu sayede rotation, crop, stretch tamamen kontrolumuzde.
        self._kamera = Camera(
            index=0,
            resolution=(1280, 720),
            size=(0, 0),           # gorunmez — texture'i biz cizecegiz
            size_hint=(None, None),
            play=True,
        )
        self.add_widget(self._kamera)

        # Kamera goruntusunu gosterecek Canvas Rectangle
        with self.canvas:
            Color(1, 1, 1, 1)
            self._cam_rect = Rectangle(pos=self.pos, size=self.size)

        # Texture degisince Rectangle'i guncelle
        def _texture_guncelle(cam, texture):
            if texture is None:
                return
            # tex_coords kombinasyonlari (birini ac, digerlerini yorum yap):
            # Normal (0 derece)      : (0,0, 1,0, 1,1, 0,1)
            # 90 CW                  : (0,1, 0,0, 1,0, 1,1)
            # 180                    : (1,1, 0,1, 0,0, 1,0)  <-- simdi bu
            # 270 CW (=90 CCW)       : (1,0, 1,1, 0,1, 0,0)
            # Yatay flip             : (1,0, 0,0, 0,1, 1,1)
            # Yatay flip + 90 CW     : (1,1, 1,0, 0,0, 0,1)
            # Yatay flip + 180       : (0,1, 1,1, 1,0, 0,0)
            # Yatay flip + 270 CW    : (0,0, 0,1, 1,1, 1,0)
            if IS_ANDROID:
                self._cam_rect.tex_coords = (1,1, 0,1, 0,0, 1,0)
            else:
                self._cam_rect.tex_coords = (0,0, 1,0, 1,1, 0,1)
            self._cam_rect.texture = texture
            self._cam_rect.pos  = self.pos
            self._cam_rect.size = self.size

        self._kamera.bind(texture=_texture_guncelle)
        # Panel yeniden boyutlaninca Rectangle'i da guncelle
        self.bind(
            pos =lambda w,v: setattr(self._cam_rect, 'pos',  self.pos),
            size=lambda w,v: setattr(self._cam_rect, 'size', self.size),
        )
